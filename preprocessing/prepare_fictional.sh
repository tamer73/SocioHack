...
prepare_dataset() {
    ...
    if [ ! -f "$PROJECT_ROOT/data/fictional/$dataset_file" ]; then
        echo "Error: Dataset file $PROJECT_ROOT/data/fictional/$dataset_file not found!"
        return 1
    fi
    
    # Run the format_dataset command
    cd "$PROJECT_ROOT" && \
    python3 -m src.format_dataset "$PROJECT_ROOT/data/fictional/$dataset_file" \
...
main() {
    ...
    local dataset_files=($(ls "$PROJECT_ROOT/data/fictional/"*.json 2>/dev/null | grep -v "copy.json" | xargs -I{} basename {}))
    
    if [ ${#dataset_files[@]} -eq 0 ]; then
        echo "No dataset files found in $PROJECT_ROOT/data/fictional directory!"
        exit 1
    fi
    ...
    for dataset_file in "${dataset_files[@]}"; do
        ...
        # must match what scripts/train_fictional.sh expects: <file>_fictional.
        local dataset_name="${dataset_file%.json}_fictional"
