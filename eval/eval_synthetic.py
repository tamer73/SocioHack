...
# --- Configuration ---
# MINED_DIR is where the RL training artifacts (loopholes_<scenario>_synthetic.json)
# live. Training writes them to the directory it is launched from (the repo root by
# default); collect them into rl_results_synthetic/ or point SOCIOHACK_MINED_DIR
# at wherever they are.
GT_DIR = os.path.join(_REPO_ROOT, "data/synthetic")
MINED_DIR = os.environ.get("SOCIOHACK_MINED_DIR", os.path.join(_REPO_ROOT, "rl_results_synthetic"))

GEMINI_MODEL_NAME = "gemini-3-flash-preview"
OUTPUT_RESULTS_FILE = os.path.join(MINED_DIR, "eval_synthetic_rl_results.json")
PAIR_CACHE_FILE = os.path.join(MINED_DIR, "eval_synthetic_rl_cache.json")
FINAL_MARKDOWN = os.path.join(MINED_DIR, "eval_synthetic_rl_summary.md")
FINAL_CSV = os.path.join(MINED_DIR, "eval_synthetic_rl_summary.csv")
...
def main():
    ...
    print(f"\n==================================================")
    print(f"========== EVALUATING SYNTHETIC (RL) ==========")
    print(f"==================================================")
    ...
    for dataset_file in dataset_files:
        ...
        # Training writes: loopholes_{dataset_name}_synthetic.json
        mined_path = os.path.join(MINED_DIR, f"loopholes_{dataset_name}_synthetic.json")
        mined_list = []
    ...
    with open(FINAL_MARKDOWN, "w", encoding="utf-8") as f_md, open(FINAL_CSV, "w", encoding="utf-8") as f_csv:
        f_md.write("# Synthetic Evaluation Results (RL, Compare vs Loopholes)\n\n")
