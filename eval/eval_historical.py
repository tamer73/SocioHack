...
# --- Configuration ---
# MINED_DIR is where the RL training artifacts (loopholes_<scenario>.json) live.
# Training writes them to the directory it is launched from (the repo root by
# default); collect them into rl_results_historical/ or point SOCIOHACK_MINED_DIR
# at wherever they are.
GT_DIR = os.path.join(_REPO_ROOT, "data/historical")
MINED_DIR = os.environ.get("SOCIOHACK_MINED_DIR", os.path.join(_REPO_ROOT, "rl_results_historical"))

GEMINI_MODEL_NAME = "gemini-3-flash-preview"
OUTPUT_RESULTS_FILE = os.path.join(_REPO_ROOT, "eval/eval_historical_rl_results.json")
PAIR_CACHE_FILE = os.path.join(_REPO_ROOT, "eval/eval_historical_rl_cache.json")
FINAL_MARKDOWN = os.path.join(_REPO_ROOT, "eval/eval_historical_rl_summary.md")
FINAL_CSV = os.path.join(_REPO_ROOT, "eval/eval_historical_rl_summary.csv")
...
def call_gemini_api_wrapped(prompt_text: str, temperature: float = 0.0) -> Tuple[str, bool]:
    try:
        # Using the centralized gemini_client from src
...
def check_gt_matches_mined_list(gt_idx: int, gt_text: str, mined_list: List[str], dataset_name: str, cache_store: dict) -> List[Dict]:
    results = []
    needed_mined_indices = []
    
    for m_idx, m_text in enumerate(mined_list):
        pair_hash = get_hash(m_text, gt_text)
        res = {
            "mined_index": m_idx,
            "mined_text": m_text,
            "gt_index": gt_idx,
...
For each Mined Strategy listed above, determine if it attempts to exploit the exact same underlying loophole, issue, or vulnerability that the Ground Truth Patch is designed to fix.
A "match" exists if they fundamentally target the same institutional vulnerability, even though one is exploiting it and the other is patching it.
        ...
        try:
            ...
            for ev in evals:
                ...
                if m_idx is not None and m_idx in needed_mined_indices:
                    ...
                    pair_hash = get_hash(mined_list[m_idx], gt_text)
...
def main():
    ...
    print(f"\n==================================================")
    print(f"========== EVALUATING HISTORICAL (RL) ==========")
    print(f"==================================================")
    ...
    for dataset_file in dataset_files:
        ...
        try:
            with open(gt_path, "r", encoding="utf-8") as f:
                gt_data = json.load(f)
                gt_list = gt_data.get("ground_truth_patch", [])
        except Exception as e:
            print(f"Error reading GT {dataset_file}: {e}")
            continue
        gt_lists[dataset_name] = gt_list
        
        mined_path = os.path.join(MINED_DIR, f"loopholes_{dataset_file}")
        mined_list = []
        ...
        if dataset_name in all_results[method] and all_results[method][dataset_name] is not None:
            ...
            if not has_error:
                if len(all_results[method][dataset_name]) == len(mined_list) * len(gt_list):
                    needs_eval = False

        if not needs_eval:
            continue

        print(f"  Processing {dataset_name} for API Judge... (GT: {len(gt_list)}, Mined: {len(mined_list)})")
        
        if not gt_list or not mined_list:
            # If either list is empty, we skip API judge and record empty results.
            # Calculation phase will see an empty results list and assign 0 score.
            all_results[method][dataset_name] = []
            save_json(OUTPUT_RESULTS_FILE, all_results)
            continue

        gt_futures = []
        dataset_matrix = [None] * len(gt_list)
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            for gt_i, gt_text in enumerate(gt_list):
                gt_futures.append(
                    executor.submit(check_gt_matches_mined_list, gt_i, gt_text, mined_list, dataset_name, pair_cache)
                )
            
            done_count = 0
            total_gt = len(gt_futures)
            
            for future in concurrent.futures.as_completed(gt_futures):
                res_list = future.result()
                if res_list:
                    g_idx = res_list[0]["gt_index"]
                    dataset_matrix[g_idx] = res_list
                done_count += 1
                if done_count % 1 == 0 or done_count == total_gt:
                    print(f"      ... evaluated {done_count}/{total_gt} GT patches", end="\r")
            print("") 
    ...
    for k in TOP_KS:
        setting_metrics = []
        for dataset_name in gt_lists.keys():
            gt_list = gt_lists[dataset_name]
            ...
            if not gt_list and not mined_list:
                ...
                setting_metrics.append({"dataset": dataset_name, "p": 0.0, "r": 0.0, "f1": 0.0, "mined": 0, "gt": len(gt_list)})
                continue
            ...
            tp_gt = len(set(m["gt_index"] for m in matches))
            tp_mined = len(set(m["mined_index"] for m in matches))
            
            recall = tp_gt / len(gt_list) if len(gt_list) > 0 else 0.0
            ...
            setting_metrics.append({
                ...
                "mined": len(mined_list), "gt": len(gt_list)
            })
    ...
    with open(FINAL_MARKDOWN, "w", encoding="utf-8") as f_md, open(FINAL_CSV, "w", encoding="utf-8") as f_csv:
        f_md.write("# Historical Evaluation Results (RL, Compare vs GT Patch)\n\n")
