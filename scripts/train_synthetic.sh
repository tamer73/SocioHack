...
create_dataset_config() {
    ...
    local config_file="configs/${dataset_name}_synthetic.yaml"
    ...
    # Substitute scenario-specific suffix and HF dataset name
    sed -i "s/suffix: \"SOCIOHACK-SYNTHETIC\"/suffix: \"SOCIOHACK-SYNTHETIC-${dataset_name}\"/" "$config_file"
    sed -i "s/name: \"sociohack_synthetic\"/name: \"${dataset_name}_synthetic\"/" "$config_file"
...
run_training() {
    local dataset_name="$1"
    local config_name="${dataset_name}_synthetic"
...
main() {
    ...
    echo "Successful: $((${#datasets[@]} - ${#failed_datasets[@]}))"
